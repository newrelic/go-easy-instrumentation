...
func main() {
	myapp, err := newrelic.NewApplication(
	...
	http.HandleFunc(newrelic.WrapHandleFunc(myapp, "/version", versionHandler))
	http.HandleFunc(newrelic.WrapHandleFunc(myapp, "/notice_error", noticeError))
	http.HandleFunc(newrelic.WrapHandleFunc(myapp, "/notice_expected_error", noticeExpectedError))
	http.HandleFunc(newrelic.WrapHandleFunc(myapp, "/notice_error_with_attributes", noticeErrorWithAttributes))
	http.HandleFunc(newrelic.WrapHandleFunc(myapp, "/custom_event", customEvent))
	http.HandleFunc(newrelic.WrapHandleFunc(myapp, "/set_name", setName))
	http.HandleFunc(newrelic.WrapHandleFunc(myapp, "/add_attribute", addAttribute))
	http.HandleFunc(newrelic.WrapHandleFunc(myapp, "/add_span_attribute", addSpanAttribute))
	http.HandleFunc(newrelic.WrapHandleFunc(myapp, "/ignore", ignore))
	http.HandleFunc(newrelic.WrapHandleFunc(myapp, "/segments", segments))
	http.HandleFunc(newrelic.WrapHandleFunc(myapp, "/mysql", mysql))
	http.HandleFunc(newrelic.WrapHandleFunc(myapp, "/external", external))
	http.HandleFunc(newrelic.WrapHandleFunc(myapp, "/roundtripper", roundtripper))
	http.HandleFunc(newrelic.WrapHandleFunc(myapp, "/custommetric", customMetric))
	http.HandleFunc(newrelic.WrapHandleFunc(myapp, "/browser", browser))
	http.HandleFunc(newrelic.WrapHandleFunc(myapp, "/async", async))
	http.HandleFunc(newrelic.WrapHandleFunc(myapp, "/message", message))
	http.HandleFunc(newrelic.WrapHandleFunc(myapp, "/log", logTxnMessage))
	...
	http.HandleFunc("/background_log", func(w http.ResponseWriter, req *http.Request) {
		...
		myapp.RecordLog(newrelic.LogData{
