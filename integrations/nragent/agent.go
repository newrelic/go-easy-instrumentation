...
func InstrumentMain(manager *parser.InstrumentationManager, c *dstutil.Cursor) {
	...
	if decl, ok := mainFunctionNode.(*dst.FuncDecl); ok {
		...
		if decl.Name.Name == "main" {
			...
			newMain, _ := parser.TraceFunction(manager, decl, tracestate.Main(manager.AgentVariableName()))
...
func checkForExistingApplicationInFunctions(manager *parser.InstrumentationManager, c *dstutil.Cursor) {
	if c == nil {
		return
	}
...
func checkFuncDeclForApplication(manager *parser.InstrumentationManager, node dst.Node) bool {
...
func handleAssignStmtForAgentVariable(manager *parser.InstrumentationManager, node dst.Node) bool {
	...
	for pos, rhs := range assign.Rhs {
		...
		if ident, ok := assign.Lhs[pos].(*dst.Ident); ok {
			manager.SetAgentVariableName(ident.Name)
			return true
		}
	}
	return false
...
func checkForExistingApplicationInMain(manager *parser.InstrumentationManager, decl *dst.FuncDecl) bool {
	if decl == nil {
		return false
	}
	...
	for _, stmt := range decl.Body.List {
		...
		if len(assign.Rhs) > 0 {
			...
			if path.Path == codegen.NewRelicAgentImportPath {
				manager.SetAgentVariableName(assign.Lhs[0].(*dst.Ident).Name)
				manager.SetSetupFunc(decl)
				return true
			}
		}
...
func NoticeError(manager *parser.InstrumentationManager, stmt dst.Stmt, c *dstutil.Cursor, tracing *tracestate.State, functionCallWasTraced bool) bool {
	if tracing.IsMain() {
		return false
	}
