...
func InstrumentChiMiddleware(manager *parser.InstrumentationManager, stmt dst.Stmt, c *dstutil.Cursor, tracing *tracestate.State) bool {
	...
	middleware, goGet := NrChiMiddleware(routerName, tracing.AgentVariable())
...
// block.
func InstrumentChiRouterLiteral(manager *parser.InstrumentationManager, stmt dst.Stmt, c *dstutil.Cursor, tracing *tracestate.State) bool {
...
// getHTTPRequestArgName extracts the HTTP request argument name from a function literal.
// It returns (true, argName) if found, (false, "") otherwise.
func getHTTPRequestArgName(fnLit *dst.FuncLit) (bool, string) {
	if fnLit == nil || fnLit.Type == nil || fnLit.Type.Params == nil {
		return false, ""
	}
	...
	for _, field := range fnLit.Type.Params.List {
		...
		if ident.Name == "Request" && ident.Path == "net/http" {
