...
func HandlerIsInstrumented(manager *parser.InstrumentationManager, fn *dst.FuncDecl) bool {
...
func isHTTPResponseWriter(respW *dst.Field) bool {
	...
	if identRespW.Path != HttpImportPath || identRespW.Name != "ResponseWriter" {
		return false
	}
	return true
}
...
func isHTTPRequest(req *dst.Field) bool {
	...
	if identReq.Path != HttpImportPath || identReq.Name != "Request" {
		return false
	}
	return true
}
	...
	if len(stmt.Rhs) == 1 && len(stmt.Lhs) == 1 && (stmt.Tok == token.DEFINE) {
		...
		if ok && unary.Op == token.AND {
			...
			if ok {
				...
				if ok && ident.Name == "Client" && ident.Path == HttpImportPath {
					return true
				}
			}
		}
...
func InstrumentHandleFunction(manager *parser.InstrumentationManager, c *dstutil.Cursor) {
	n := c.Node()
	...
	if isFn && IsHTTPHandler(fn) && !HandlerIsInstrumented(manager, fn) {
		...
		newFn, ok := parser.TraceFunction(manager, fn, tracestate.FunctionBody(txnName))
		if ok {
...
func InstrumentHttpClient(manager *parser.InstrumentationManager, c *dstutil.Cursor) {
	n := c.Node()
	...
	case *dst.AssignStmt, *dst.ExprStmt:
		dst.Inspect(node, func(n dst.Node) bool {
			...
			if ok {
				...
				if ok && ident.Path == HttpImportPath {
					switch ident.Name {
...
func CannotInstrumentHttpMethod(manager *parser.InstrumentationManager, c *dstutil.Cursor) {
	n := c.Node()
...
func GetHttpResponseVariable(manager *parser.InstrumentationManager, stmt dst.Stmt) dst.Expr {
...
func ExternalHttpCall(manager *parser.InstrumentationManager, stmt dst.Stmt, c *dstutil.Cursor, tracing *tracestate.State) bool {
	if c.Index() < 0 {
		return false
	}
	pkg := manager.GetDecoratorPackage()
...
func WrapNestedHandleFunction(manager *parser.InstrumentationManager, stmt dst.Stmt, c *dstutil.Cursor, tracing *tracestate.State) bool {
	wasModified := false
	pkg := manager.GetDecoratorPackage()
	dst.Inspect(stmt, func(n dst.Node) bool {
		switch v := n.(type) {
...
func DetectWrappedRoutes(manager *parser.InstrumentationManager, c *dstutil.Cursor) {
